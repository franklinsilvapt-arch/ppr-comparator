(function () {
  ...
  <div class="lpc-card">
    ...
    <div class="lpc-calculate-row">
      <button type="button" class="lpc-calculate-btn" id="lpc-calculate-btn">Calcular</button>
    </div>
  </div>
...
(function () {
  ...
  // v3: state.committed distingue "selecção feita" de "resultado
  ...
  function selectFundInSlot(slotIdx, id) {
    ...
    // v3: alterar selecção invalida o último cálculo. Chart/tabelas
    // voltam a estado vazio até nova acção no botão Calcular.
  ...
  // v3: devolve vazio quando o utilizador ainda não calculou (ie.
  // seleccionou mas ainda não carregou em "Calcular"). Assim o chart e
  // as tabelas mantêm estado vazio até haver acção explícita.
  ...
  function renderCompareTable() {
    ...
    head.innerHTML = '<th></th>' + pairs.map(function (p) {
      ...
      return '<th class="lpc-fund-col">'
        ...
        + '<span class="lpc-fund-manager">' + (p.fund.manager || '') + '</span>'
  ...
  // v3: botão Calcular. Commit da selecção + render completo. Estado
  // visual (disabled/enabled) reflecte se há pelo menos 1 PPR escolhido.
  function updateCalculateBtnState() {
    var btn = document.getElementById('lpc-calculate-btn');
    if (!btn) return;
    var anySelected = state.selected.some(function (id) { return !!id; });
    btn.disabled = !anySelected;
    btn.classList.toggle('is-dirty', anySelected && !state.committed);
  }
  var calculateBtn = document.getElementById('lpc-calculate-btn');
  if (calculateBtn) {
